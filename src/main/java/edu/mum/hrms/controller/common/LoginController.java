 ...
 */
@Controller
@SessionAttributes(edu.mum.hrms.util.AppConstants.SYS_USER_KEY_IN_SESSION)
...
public class LoginController extends LcfBaseController{
	...
	public String login(Model model, @RequestParam("loginName") String loginName, String password, @ModelAttribute SysUser user, BindingResult errorResult) {
		try {
			...
			SysUser currentUser = userManageService.getAuthenticatedUser(loginName, password);

			if(currentUser == null){
				log.debug("@@initLogin user is null!!!!!!!!!!");
				super.addFieldError("loginName", "Expected valid loginName or password.", errorResult);
				return "login";
			}
