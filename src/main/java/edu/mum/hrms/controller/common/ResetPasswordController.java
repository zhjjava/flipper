...
public class ResetPasswordController extends LcfBaseController {
	...
	public String sendRequestOfResetPassword(@ModelAttribute SysUser sysUser, BindingResult bindingResult) {
		...
		// try {
		// emailService.sendResetPasswordMail(sysUser.getFirstName(),
		// receiptEmail, sysUser, new Locale("en"));
		//
...
//				
		...
		try {
			SysUser user = userService.get(sysUser.getUserId());
			...
			credentialsService.update(userCredentials);
		} catch (AppException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return "showResetPasswordPage";
